describe('Participation', () => {
  ...
  it('cannot create participation when shift is full', () => {
    cy.task('queryDatabase', {
      query: 'UPDATE SHIFT SET participants_limit = 0 WHERE activity_id = 1',
      credentials: {
    ...
    cy.get('[data-cy="participantsNumberInput"]').type(3);
    cy.get('[data-cy="participantsReviewInput"]').type(
      'The volunteer did a good job',
    );

    cy.get('[data-cy="participantsShiftSelect"]').click();
    cy
      .contains(
    ...
    cy.get('@participation.all').should('have.length', 0);
    cy.logout();
  });
