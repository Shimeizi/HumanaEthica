...
export default class RemoteServices {
  ...
  static async createParticipation(
    ...
    if (!participation.shiftId || !participation.enrollmentId) {
      throw Error('Shift and enrollment are required to create participation');
    }

    return httpClient
      .post(
        `/participations/${participation.shiftId}/enrollment/${participation.enrollmentId}`,
        participation,
      )
      .then((response) => {
  ...
  static async getShiftsByActivity(activityId: number): Promise<Shift[]> {
    return httpClient
      ...
      .then((response) => {
        return response.data.map((shift: any) => new Shift(shift));
      })
      .catch(async (error) => {
        throw Error(await this.errorMessage(error));
      });
